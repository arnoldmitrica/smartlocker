...
class Fullscreen_Window:
	...
	def returnToIdle_fromLockUnv(self):
				...
				if event.type==1 and event.value==1:
						if event.code==28:
							...
							if cur.rowcount != 1:
								...
								numelist = user_info['name']
								#print(user_info['name'])
								global user_infoglb
								user_infoglb=user_info
	...
	def codeInput(self, value, userPin, mobileNumber):
		...
		if pinLength == 6:
			...
			print("\n")
			print(numelist)
			cur.execute("SELECT * FROM access_locker WHERE nume = '%s' OR nume='%s'"  % ("",numelist))
			dbConnection.commit()
			print("Verificare numar locker inainte de SMS")
			print(cur.rowcount)
			if pin == userPin and cur.rowcount!=0:
				...
				self.PINresultLabel = ttk.Label(self.tk, text="There is no available cabinet\nat this time.")
				self.PINresultLabel.config(font='size, 20', justify='center', anchor='center')
	...
	def smsCodeEnteredInput(self, value, smsCode):
		...
		if smsCodeEnteredLength == 6:
			...
			if smsCodeEntered == smsCode:
				dbConnection = MySQLdb.connect(host=dbHost, user=dbUser, passwd=dbPass, db=dbName)
				cur = dbConnection.cursor()
				userName = user_infoglb['name']
				cur.execute("SELECT * FROM access_locker WHERE nume = '%s'" % (userName))
				if cur.rowcount==1:
					user_locker = cur.fetchone()
					print("\nA ajuns pe ramura sa iti STEARGA numele din lista acces locker")
					#userlockerid = user_locker['id'-1]
					self.welcomeLabel.config(text="Numele adaugat")
					print(user_locker[0])
					cur.execute("UPDATE access_locker SET nume = '%s' WHERE id='%s'" % ("",user_locker[0]))
					dbConnection.commit()
				else:
					print("A ajuns la cursor\n")
					cur.execute("SELECT * FROM access_locker WHERE nume = '%s'" % (""))
					if cur.rowcount!=0:	
						user_locker= cur.fetchone()
						print("\nA ajuns pe ramura sa iti ADAUGE numele im lista acces locker")
						cur.execute("UPDATE access_locker SET nume = '%s' WHERE id='%s'" % (userName,user_locker[0]))
						dbConnection.commit()
				...
				if user_locker[0]==1 :
					print("Cabinet number 1 is open on GPIO 16")
					GPIO.output(20,GPIO.LOW)
					
				if user_locker[0]==2 :
					print("Cabinet number 2 is open on GPIO 26")
					GPIO.output(21,GPIO.LOW)
					
				if user_locker[0]==3 :
					print("Cabinet number 3 is open on GPIO 29")
